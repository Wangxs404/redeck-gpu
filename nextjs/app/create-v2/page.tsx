...
interface Project {
  ...
  // 新增：结构化 JSON 数据（用于前端编辑）
  slideData?: SlideData;
  // 保留：HTML 预览（用于 iframe 展示和 PPTX 转换）
...
export default function CreatePage() {
  ...
  const startNewProject = async (file: File, modelId: string) => {
    ...
    try {
      ...
      // 3. Generate Slide JSON (新接口，同时返回 HTML 预览)
      const jsonRes = await fetch(`${API_BASE}/slides/json`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          image_path: uploadData.file_path, 
          model: modelId,
          render_html: true  // 同时生成 HTML 用于预览
        }),
      });
      
      const jsonData = await jsonRes.json();
      if (!jsonData.success) throw new Error(jsonData.message || '生成失败');
      
      updateProject(newId, { 
        status: 'success',
        // 存储结构化 JSON 数据（可用于前端编辑）
        slideData: jsonData.slide,
        // 同时保存 HTML 预览（用于 iframe 展示和 PPTX 转换）
        htmlRes: jsonData.html ? {
          html: jsonData.html,
          html_file_path: jsonData.html_file_path,
  ...
  const regenerate = async (id: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/slides/json`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
               ...
               <>
                 ...
                 {activeProject.slideData && (
                   <span className="text-xs text-zinc-500">
                     {activeProject.slideData.elements?.length || 0} 元素
                   </span>
                 )}
               </>
             )}
        ...
        <div className="flex-1 relative overflow-hidden flex items-center justify-center p-8">
          ...
          <AnimatePresence mode="wait">
            ...
            {activeProject.status === 'success' && (
              ...
              >
                {activeProject.htmlRes ? (
                  // 有 HTML 时使用 iframe 预览
                  <div className="relative w-[90%] h-[90%] bg-white rounded-lg shadow-2xl overflow-hidden border border-white/10 ring-1 ring-black/50">
                     <iframe 
                  ...
                  // 只有 JSON 时显示数据概览
                  <div className="w-[90%] h-[90%] bg-zinc-900 rounded-lg border border-white/10 p-8 overflow-auto">
                    <div className="mb-4 flex items-center gap-4">
                      <h3 className="text-lg font-semibold text-white">Slide JSON Data</h3>
                      <span className="text-xs text-zinc-500 bg-zinc-800 px-2 py-1 rounded">
                        {activeProject.slideData.width} × {activeProject.slideData.height} pt
                      </span>
                    </div>
                    <pre className="text-xs text-zinc-400 font-mono whitespace-pre-wrap">
                      {JSON.stringify(activeProject.slideData, null, 2)}
                    </pre>
                  </div>
                ) : (
                  <div className="text-zinc-500">无预览数据</div>
                )}
              </motion.div>
            )}
