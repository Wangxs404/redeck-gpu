...
export async function POST(request: NextRequest) {
  try {
    ...
    // 调用 FastAPI 图片识别接口
    const response = await fetch(`${FASTAPI_URL}/recognize`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        prompt: prompt || 'Please analyze this image and provide a detailed description of what you see.'
      }),
    })
    ...
    console.error('图片识别 API 路由错误:', error)
